...
(function () {
    ...
    const UserAPI = {
        // Get user profile
        async getProfile() {
            const sessionId = storage.getItem('sessionToken');
    ...
    const AdminAPI = {
        ...
        async getUsers(page = 1, limit = 20, search = '') {
            const sessionId = storage.getItem('sessionToken');
            if (!sessionId) throw new Error('Nicht eingeloggt');
            return await apiRequest(`/admin/users?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`);
        },

        // Promote user to admin
        async promoteUser(userId, password) {
            const sessionId = storage.getItem('sessionToken');
            if (!sessionId) throw new Error('Nicht eingeloggt');
            return await apiRequest(`/admin/users/${userId}/promote`, {
                method: 'PUT',
        ...
        // Verify user
        async verifyUser(userId, verified, password) {
            const sessionId = storage.getItem('sessionToken');
            if (!sessionId) throw new Error('Nicht eingeloggt');
            return await apiRequest(`/admin/users/${userId}/verify`, {
                method: 'PUT',
                body: JSON.stringify({ verified, admin_password: password })
            });
        },

        // Update user
        async updateUser(userId, data, password) {
            const sessionId = storage.getItem('sessionToken');
            if (!sessionId) throw new Error('Nicht eingeloggt');
            return await apiRequest(`/admin/users/${userId}`, {
                method: 'PUT',
                body: JSON.stringify({ ...data, admin_password: password })
            });
        },

        // Reset user password
        async resetUserPassword(userId, newPassword, adminPassword) {
            const sessionId = storage.getItem('sessionToken');
            if (!sessionId) throw new Error('Nicht eingeloggt');
